...
def main():
    print("ASL Training Utility")
    print("Enter a label (letter/word), then make the gesture with the glove. Press Enter to record.")
    
    try:
        ser = serial.Serial('COM3', 115200, timeout=2)
        print(f"Connected to ESP32 on COM3")
        
        with open('asl_training_data.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            
            # Write header if file is empty
            if csvfile.tell() == 0:
                writer.writerow(['label', 'flex1', 'flex2', 'flex3', 'flex4', 'flex5', 
                               'accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z'])
            
            while True:
                label = input("Enter label (or 'exit' to quit): ").strip()
                if label.lower() == 'exit':
                    break
                    
                input(f"Make the gesture for '{label}', then press Enter to record...")
                ser.reset_input_buffer()
                print("Reading sensor data...")
                
                # Read multiple lines to get a stable value
                readings = []
                for _ in range(5):
                    line = ser.readline().decode('utf-8').strip()
                    if line and line != "UNSTABLE":
                        readings.append(line)
                    time.sleep(0.1)
                
                if readings:
                    # Save all readings for this label
                    for r in readings:
                        writer.writerow([label] + r.split(','))
                    print(f"Saved {len(readings)} samples for label '{label}'.")
                else:
                    print("No stable data received. Try again.")
